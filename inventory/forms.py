...
class ProductForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and not user.is_superuser:
            # We remove cost_price from the fields or make it read-only and hide it.
            # Hiding and making read-only is cleaner to prevent validation errors for empty fields.
            self.fields['cost_price'].required = False
            self.fields['cost_price'].widget = forms.HiddenInput()
            # Alternatively, if we are editing an existing product, we keep the existing cost_price.
